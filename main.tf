...
resource "aws_iam_role" "scheduled_task_ecs_execution" {
  ...
  assume_role_policy = file(
    "${path.module}/policies/scheduled-task-ecs-execution-assume-role-policy.json",
  )
}

data "template_file" "scheduled_task_ecs_execution_policy" {
  template = file(
    "${path.module}/policies/scheduled-task-ecs-execution-policy.json",
  )
}
...
resource "aws_iam_role" "scheduled_task_ecs" {
  ...
  assume_role_policy = file(
    "${path.module}/policies/scheduled-task-ecs-assume-role-policy.json",
  )
}
...
resource "aws_iam_role" "scheduled_task_cloudwatch" {
  ...
  assume_role_policy = file(
    "${path.module}/policies/scheduled-task-cloudwatch-assume-role-policy.json",
  )
}

data "template_file" "scheduled_task_cloudwatch_policy" {
  template = file(
    "${path.module}/policies/scheduled-task-cloudwatch-policy.json",
  )
